  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        ...
        const q = query(userDoc, where('uid', '==', userID));
        const querySnapshot = await getDocs(q);
    
        if (!querySnapshot.empty) {
          const user = querySnapshot.docs[0].data(); // Get the data of the first document
    ...
    <>
      <div className='card mb-5 mb-xl-10' id='kt_profile_details_view'>
        <div className='card-header cursor-pointer'>
          ...
          <Link to={`/crafted/pages/profile/settings/${userID}`} className='btn btn-primary align-self-center'>
            Edit Profile
          </Link>
        </div>
