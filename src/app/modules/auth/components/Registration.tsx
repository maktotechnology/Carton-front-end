...
const sendVerificationEmail = async (userEmail) => {
  try {
    ...
    // Use EmailJS to send the verification email
    const result = await emailjs.send(
      'service_5sgtx4h',
      'template_v6zd109',
      {
        public_key: 'cI2D01_IBXJYla9Po',
        to_email: userEmail,
        otp: otp,
      },
    );
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setStatus, setSubmitting }) => {
      ...
      try {
        ...
        let registrationData = storedData ? JSON.parse(storedData) : [];
        ...
        localStorage.setItem('registerData', JSON.stringify(registrationData));
